...
Implements User-Based Collaborative Filtering using Surprise KNNBasic.
Predicts ratings based on similar users using cosine similarity.
"""
...
def train_user_based_model(train_df: pd.DataFrame) -> KNNBasic:
    """
    Train User-Based Collaborative Filtering model.

    Args:
        train_df (pd.DataFrame): Must contain:
            user_id, item_id, rating

    Returns:
        KNNBasic: Trained Surprise model
    """
    ...
    logger.info(f"Surprise users: {trainset.n_users}")
    logger.info(f"Surprise items: {trainset.n_items}")
    logger.info(f"Surprise ratings: {trainset.n_ratings}")
    ...
    logger.info(f"k neighbors: {DEFAULT_K_NEIGHBORS}")
...
def get_all_predictions(
    ...
    """
    Predict ratings for entire test set.
...
if __name__ == "__main__":
    ...
    df = data_loader.load_ratings(str(data_path))

    train_df, test_df = train_test_split.temporal_train_test_split(df)
